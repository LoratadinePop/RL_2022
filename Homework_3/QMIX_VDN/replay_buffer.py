...
class ReplayBuffer:
    def __init__(self, args):
        ...
        self.buffer = {
            'obs_n': np.zeros(
                [self.buffer_size, self.episode_limit + 1, self.N, self.obs_dim]
            ),
            's': np.zeros([self.buffer_size, self.episode_limit + 1, self.state_dim]),
            # Note: We use 'np.ones' to initialize 'avail_a_n'
            #    'avail_a_n': np.ones([self.buffer_size, self.episode_limit + 1, self.N, self.action_dim]),
            'last_onehot_a_n': np.zeros(
                [self.buffer_size, self.episode_limit + 1, self.N, self.action_dim]
            ),
    ...
    def store_transition(self, episode_step, obs_n, s, last_onehot_a_n, a_n, r, dw):
        self.buffer['obs_n'][self.episode_num][episode_step] = obs_n
        self.buffer['s'][self.episode_num][episode_step] = s
        # self.buffer['avail_a_n'][self.episode_num][episode_step] = avail_a_n
        self.buffer['last_onehot_a_n'][self.episode_num][
            episode_step + 1
        ] = last_onehot_a_n
        ...
        for key in self.buffer.keys():
            if key == 'obs_n' or key == 's' or key == 'last_onehot_a_n':
                batch[key] = torch.tensor(
                    self.buffer[key][index, :(max_episode_len + 1)], dtype=torch.float32
                )
            elif key == 'a_n':
                batch[key] = torch.tensor(
                    self.buffer[key][index, :max_episode_len], dtype=torch.long
                )
            else:
                batch[key] = torch.tensor(
                    self.buffer[key][index, :max_episode_len], dtype=torch.float32
                )
