...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	delta = tol + 1
	# Iterate until converge to the desired tol
	while delta > tol:
		delta = 0
		# Iterate over all states
		for state in np.arange(nS):
			pre_state_value = value_function[state]
			# evaluate_policy_for_state
			state_value = 0
			action = policy[state]
			...
			delta = max(delta, np.abs(pre_state_value - value_function[state]))
	############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
	...
	# YOUR IMPLEMENTATION HERE #
	for state in np.arange(nS):
		# improve_policy_for_state
		state_value_max = float("-inf")
		for action in np.arange(nA):
			state_value = 0
			for transition in P[state][action]:
				prob, next_state, reward, _ = transition
				state_value += prob * (reward + gamma * value_from_policy[next_state])
			if state_value > state_value_max:
				state_value_max = state_value
				action_max = action
		new_policy[state] = action_max
	############################
	return new_policy
		...
		for state in np.arange(nS):
			...
			for action in np.arange(nA):
				...
				for transition in P[state][action]:
					prob, next_state, reward, _ = transition
					state_value += prob * (reward + gamma * value_function[next_state])
				value_function[state] = max(state_value, value_function[state])
			delta = max(delta, np.abs(pre_state_value - value_function[state]))
